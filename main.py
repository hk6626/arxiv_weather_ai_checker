...
def evaluate_and_summarize(entry, themes, model_name):
    """
...
"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            # 429 Too Many Requests や Resource Exhausted などのレートリミットエラーを検知
            if "429" in error_msg or "quota" in error_msg or "exhausted" in error_msg or "too many requests" in error_msg:
                if attempt < max_retries - 1:
                    wait_time = 60
                    print(f"Rate limit hit. Waiting for {wait_time} seconds before retrying (Attempt {attempt+1}/{max_retries})...")
...
def main():
    ...
    for entry in entries:
        ...
        # 判定と要約
        summary_text = evaluate_and_summarize(entry, themes, model_name)
